...
Agent skills for [prisma-ltree](https://github.com/slovakian/prisma-ltree) — typed PostgreSQL `ltree` support in [Prisma 8 / Prisma Next](https://github.com/prisma/prisma) apps.

> **Install the skills version that matches your `prisma-ltree` and `@prisma/orm-*` pins.** The skill cluster documents the extension surface at the framework version the package was validated against (today `@prisma/orm-*@8.0.0-rc.1`).
...
That installs `prisma-8` (usage), `prisma-next-upgrade` (app upgrades), and `prisma-8-extension-upgrade` (extension authors). Live source: [`prisma/prisma/skills`](https://github.com/prisma/prisma/tree/main/skills).
...
Skills install at the **project level** (same as upstream Prisma skills). For a single agent runtime, swap `--all` for `-a <agent>` (e.g. `-a cursor`, `-a claude-code`).
...
| `@prisma/orm-*@…`  | Framework pin baked into `prisma-ltree`'s `package.json` — must match your app |
| Skills git ref     | Should track the same release as `prisma-ltree` when possible                  |

Before upgrading `@prisma/orm-*` past the extension's pinned version, check for a newer `prisma-ltree` release. The upstream `prisma-next-upgrade` skill enforces extension pins automatically.
