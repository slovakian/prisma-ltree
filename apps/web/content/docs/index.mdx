...
Scalar `ltree` query operators and functions are essentially complete. See
[Limitations](/docs/limitations) for GiST indexes, boolean `ltree[]` operators,
`lca(ltree[])`, and other deliberate gaps.
