...
- The export surface is **byte-for-byte canonical** with the Prisma 8 reference
  ...
  has wired any Prisma 8 extension already knows how to wire this one. Do not
...
| ------------------------------ | -------------------------------------------------------------------- | ------------------------------------------------------------------------------- | ------------------------------------------------------------------------ |
| `prisma-ltree/control`         | `ltreeExtensionDescriptor`: **default** _and_ named                 | `import ltree from "prisma-ltree/control"`                                      | `prisma.config.ts` → `definePrismaConfig({ orm: ormConfig({ extensions: [ltree] }) })` |
| `prisma-ltree/runtime`         | `ltreeRuntimeDescriptor` (**default**); `ltreeCodecRegistry` (named) | `import ltree from "prisma-ltree/runtime"`                                      | execution stack / facade → `extensions: [ltree]`                         |
| `prisma-ltree/pack`            | `ltreePackMeta`: **default only**                                   | `import ltreePack from "prisma-ltree/pack"`                                     | contract → `defineContract({ extensions: { ltree: ltreePack } })`        |
...
The import _syntax_ is the smaller part; what trips people up is remembering which export goes
where. There are exactly three places:

1. **`prisma.config.ts`** (migrate/CLI plane) → `control`
