...
| GiST index (`gist_ltree_ops`, default siglen) | DDL                   | supported    | `type: "gist"` on `ltree` columns via pack `indexTypes`; PSL↔TS parity in `test/psl-lane/` (ADR-006) |
| GiST array index (`gist__ltree_ops`, default siglen) | DDL            | supported    | same — PostgreSQL selects opclass from `ltree[]` column type                                           |
| GiST `siglen` / explicit opclass              | DDL                   | planned      | Operator-class params not modeled in prisma-next general index DSL (ADR-006)                           |
...
- 2026-07-08 — GiST indexes (`type: "gist"`) on `ltree` / `ltree[]` columns → `supported` with pack `indexTypes`, PSL↔TS parity test, and PGlite integration (ADR-006). Custom `siglen` / explicit operator class remains `planned` (prisma-next opclass gap).
